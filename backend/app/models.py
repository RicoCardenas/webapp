...
class Users(db.Model):
    ...
    managed_teachers = db.relationship('AdminTeacherAssignment', back_populates='admin', foreign_keys='AdminTeacherAssignment.admin_id', cascade="all, delete-orphan", lazy='selectin')
    managed_teacher_assignment = db.relationship('AdminTeacherAssignment', back_populates='teacher', foreign_keys='AdminTeacherAssignment.teacher_id', uselist=False)
    managed_teacher_groups = db.relationship('AdminTeacherGroup', back_populates='admin', foreign_keys='AdminTeacherGroup.admin_id', cascade="all, delete-orphan", lazy='selectin')
...
class AdminTeacherAssignment(db.Model):
    __tablename__ = 'admin_teacher_assignments'

    id = db.Column(GUID(), primary_key=True, server_default=func.gen_random_uuid())
    admin_id = db.Column(GUID(), db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    teacher_id = db.Column(GUID(), db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, unique=True, index=True)
    assigned_at = db.Column(db.DateTime(timezone=True), nullable=False, server_default=func.now())

    admin = db.relationship('Users', foreign_keys=[admin_id], back_populates='managed_teachers')
    teacher = db.relationship('Users', foreign_keys=[teacher_id], back_populates='managed_teacher_assignment')


class AdminTeacherGroup(db.Model):
    __tablename__ = 'admin_teacher_groups'
    ...
    admin = db.relationship('Users', foreign_keys=[admin_id], back_populates='managed_teacher_groups')
    members = db.relationship('AdminTeacherGroupMember', back_populates='group', cascade="all, delete-orphan", lazy='selectin')

    __table_args__ = (
        db.UniqueConstraint('admin_id', 'name', name='uq_admin_teacher_group_name'),
    )


class AdminTeacherGroupMember(db.Model):
    __tablename__ = 'admin_teacher_group_members'

    id = db.Column(GUID(), primary_key=True, server_default=func.gen_random_uuid())
    group_id = db.Column(GUID(), db.ForeignKey('admin_teacher_groups.id', ondelete='CASCADE'), nullable=False, index=True)
    teacher_id = db.Column(GUID(), db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    added_at = db.Column(db.DateTime(timezone=True), nullable=False, server_default=func.now())

    group = db.relationship('AdminTeacherGroup', back_populates='members')
    teacher = db.relationship('Users', foreign_keys=[teacher_id])

    __table_args__ = (
        db.UniqueConstraint('group_id', 'teacher_id', name='uq_admin_teacher_group_member'),
    )
